...
function  makeStreamData(data,featureOrder){
    var typeSet = new Set();
    ...
    {
        typeSet.add(data[i][typeOrder]);
        featureSet.add(data[i][featureOrder]);
    }
    var features = Array.from(featureSet).sort();
    var types = Array.from(typeSet).sort();
    var minfeature = d3.min(features);
    var maxfeature = d3.max(features);
    ...
    for(var i = 0;i<data.length;i++)
    {
        ...
        {
            if(streamData[j].feature==data[i][featureOrder].toFixed(1) )
                streamData[j][data[i][typeOrder]]++;
        }
    }
