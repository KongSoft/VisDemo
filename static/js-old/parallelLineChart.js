...
function drawParallelLineChart(data){
    /* ----------------------------配置参数------------------------  */
    const chart = new Chart();
    const config = {
        lineColor: chart._colors(0),
        margins: {top: 80, left: 10, bottom: 50, right: 10},
        textColor: 'black',
        title: '平行坐标系折线图',
        hoverColor: 'red',
        padding: 80,
    ...
    chart.renderLines = function(){
        ...
        const linesEnter = lines.enter()
              ...
              linesEnter.append('path')
                            .attr('stroke', (d,i) => chart._colors(d.name))
                            .attr('stroke-width', 2)
                            .attr('fill', 'none')
    ...
    chart.box(d3.select("#parallelLineView"));
    chart.renderChart();
